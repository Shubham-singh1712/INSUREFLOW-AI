...
const packetSections = ['Claim summary cover page', 'AI verification page', 'Document index with bookmarks', 'Insurance card', 'Discharge summary', 'Invoices and lab reports'];
const packets = [
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <MetricCard label="Generated Today" value={demoMode.enabled ? '9' : '0'} helper="Master claim packets" tone="success" />
        <MetricCard label="Draft Packets" value={demoMode.enabled ? '4' : '0'} helper="Waiting on final validation" tone="warning" />
        <MetricCard label="Avg Pages" value={demoMode.enabled ? '37' : '0'} helper="Per claim packet" tone="info" />
        ...
        <div className="card p-5">
          ...
          {visibleSections.length === 0 && (
            <div className="p-8 text-center text-muted-foreground">
              No live packet template has been loaded yet. Turn on Demo Mode in Settings to view the mock builder.
            </div>
          )}
        </div>
        <div className="card p-5">
          ...
          {visiblePackets.length === 0 && (
            <div className="p-8 text-center text-muted-foreground">
              No live packets have been generated yet. Turn on Demo Mode in Settings to view mock packets.
            </div>
          )}
          {demoMode.enabled && (
