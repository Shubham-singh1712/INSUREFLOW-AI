    ...
    <div className="card overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {filtered.length} claims - {claims.filter((claim) => claim.repairStatus === 'ocr_failed' || claim.repairStatus === 'signature_missing').length} need attention
          </p>
        </div>
                      ...
                      <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full confidence-fill ${
                            claim.aiConfidence >= 85 ? 'bg-success' : claim.aiConfidence >= 65 ? 'bg-warning' : 'bg-danger'
                          }`}
                          style={{ width: `${claim.aiConfidence}%` }}
                        />
                      </div>
                      <span className={`text-xs font-semibold font-tabular ${
                        claim.aiConfidence >= 85 ? 'text-success-foreground' : claim.aiConfidence >= 65 ? 'text-warning-foreground' : 'text-danger-foreground'
                      }`}>
                        {claim.aiConfidence}%
                      </span>
                    </div>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-1.5">
                      <div className={`w-2 h-2 rounded-full ${
                        claim.submissionScore >= 85 ? 'bg-success' : claim.submissionScore >= 60 ? 'bg-warning' : 'bg-danger'
                      }`} />
            ...
            {paginated.length === 0 && (
              <tr>
                <td className="px-5 py-10 text-center text-muted-foreground" colSpan={11}>
                  No live claims are loaded yet. Turn on Demo Mode in Settings to view mock claim data.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-t border-border">
        <p className="text-xs text-muted-foreground font-tabular">
          Showing {sorted.length === 0 ? 0 : (page - 1) * perPage + 1}-{Math.min(page * perPage, sorted.length)} of {sorted.length} claims
        </p>
