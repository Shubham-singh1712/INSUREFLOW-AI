...
export default function DocumentUploadStep({ onNext }: DocumentUploadStepProps) {
    ...
    readFileAsDataUrl(file)
      .then((dataUrl) => {
        setUploadedDocs((prev) => ({
          ...prev,
          [docId]: prev[docId] ? { ...prev[docId], dataUrl } : prev[docId],
        }));
      })
      .catch(() => {
        setUploadedDocs((prev) => ({
          ...
          [docId]: prev[docId]
            ? {
                ...
                status: 'failed',
                progress: 100,
                message: 'Could not read document content.',
              }
            : prev[docId],
        }));
      });
      ...
      if (progress >= 100) {
        ...
        setUploadedDocs((prev) => ({
          ...
          [docId]: {
            ...
            progress: 100,
            status: 'processing',
            message: 'Preparing document for AI extraction...',
          },
        }));

        setTimeout(() => {
          const outcome: UploadedDoc['status'] = file.size < 8 * 1024 ? 'warning' : 'passed';
          const messages: Record<string, string> = {
            passed: 'Document ready for AI extraction',
            warning: 'Small file detected. AI extraction may need review',
            failed: 'Document unreadable',
          };
          setUploadedDocs((prev) => ({
            ...prev,
            [docId]: {
  ...
  const uploadedRequired = requiredDocs.filter(
    (d) => uploadedDocs[d.id]?.status === 'passed' || uploadedDocs[d.id]?.status === 'warning'
  );
      ...
      <div className="card p-4 mb-6 flex items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground font-tabular">
              {totalUploaded} / {documentTypes.length}
            </p>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground font-tabular">
              {
                Object.values(uploadedDocs).filter(
          ...
          return (
            ...
            >
              ...
              <div className="px-4 pt-4 pb-3 flex items-start gap-3">
                <div
                  className={`w-9 h-9 rounded-xl ${docType.iconBg} flex items-center justify-center shrink-0`}
                >
                ...
                {!uploaded ? (
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground">
                      {docType.acceptedFormats} · Max 20MB
                    </p>
                    <input
                      ref={(el) => {
                  ...
                  <div
                    className={`rounded-xl border p-3 ${
                      uploaded.status === 'passed'
                        ? 'bg-success-bg/40 border-success/20'
