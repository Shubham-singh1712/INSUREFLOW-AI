...
const fieldDefinitions: Array<Omit<ClaimField, 'value' | 'confidence' | 'source'> & { patterns: RegExp[] }> = [
  {
    id: 'patientName',
    label: 'Patient name',
    patterns: [
  ...
  },
  {
    ...
    patterns: [
      ...
      /(Dr\.?\s*[A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){0,3})/i,
    ],
  },
  {
    id: 'hospital',
...
const countPattern = (text: string, pattern: RegExp) => [...text.matchAll(pattern)].length;
...
const findDateByLabel = (text: string, label: string) =>
  cleanValue(
    text.match(new RegExp(`${label}\\s*[:\\-]?\\s*(\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4}|\\d{4}-\\d{2}-\\d{2})`, 'i'))?.[1] ||
      ''
  );
...
const inferDocumentGroups = (text: string, pages: number, ocrConfidence: number): DocumentGroup[] => {
...
const buildLocalExtraction = (text: string, pages: number, filename: string) => {
  ...
  const fields: ClaimField[] = fieldDefinitions.map((definition) => {
    ...
    return {
      ...
      value:
        definition.id === 'invoiceTotal'
          ? formatMoney(rawValue)
          : rawValue || emptyValue,
      confidence,
      source: rawValue ? `${filename} - PDF text extraction` : `${filename} - not found in extracted text`,
    };
  });
  ...
  return {
    ...
    facts: {
      ...
      hasSignature: hasAny(text, [/signature/i, /signed\s+by/i, /e[-\s]?signed/i, /authorized\s+signature/i]),
...
const buildLocalIssues = (text: string, fields: ClaimField[], facts: ReturnType<typeof buildLocalExtraction>['facts'], ocrConfidence: number) => {
  ...
  if (!facts.hasDob) {
    issues.push({
      ...
      evidence: 'No DOB, Date of Birth, or Birth Date label was found in the extracted packet text.',
    });
  }
  ...
  if (!facts.hasDiagnosis) {
    issues.push({
      ...
      evidence: 'No diagnosis label or ICD-10 diagnosis pattern was extracted from the uploaded PDF.',
    });
  }
  ...
  if (!facts.hasSignature) {
    issues.push({
      ...
      evidence: 'The extracted text has no signature, signed-by, e-sign, or authorized-signature marker.',
    });
  }

  if (!facts.hasInsuranceId || insurance === emptyValue || insurance.replace(/[^A-Z0-9]/gi, '').length < 8) {
    issues.push({
  ...
  if (!facts.hasInvoice) {
    ...
    issues.push({
      ...
      fix: `Reconcile line items before submission. Extracted line items sum to INR ${Math.round(
        candidateLineSum
      ).toLocaleString('en-IN')} while the total is INR ${Math.round(invoiceTotal).toLocaleString('en-IN')}.`,
      evidence: 'The extracted numeric line items do not reconcile with the detected invoice total.',
    });
  }

  if (ocrConfidence < 58 || text.length < 250 || /blur|blurry|illegible|unreadable|low\s+quality/.test(textLower)) {
    issues.push({
      id: 'low-scan-quality',
...
const buildMetrics = (issues: ValidationIssue[], ocrConfidence: number): Pick<ValidationReport, 'metrics' | 'readinessScore' | 'healthScore'> => {
  ...
  const highRisk = issues.some((issue) => issue.severity === 'Critical' || issue.severity === 'High');
...
const buildLocalReport = ({
  ...
  return {
    ...
    timeline: [
      ...
      { id: 'parsed', label: extractionMethod === 'pdf_text' ? 'PDF Text Parsed' : 'OCR Required', time: now, done: true },
      ...
      { id: 'ready', label: 'Submission Ready', time: readinessScore >= 85 ? now : 'Pending repairs', done: readinessScore >= 85 },
    ],
    pdfStructure: [
        ...
        messages: [
          ...
          },
          {
            ...
            content: JSON.stringify({
              task:
                'Validate this claim packet for missing DOB, invoice mismatch, missing signatures, incomplete insurance ID, blurry scans, inconsistent totals, missing pages, missing diagnosis, date logic, payer/claim readiness, and any other claim-specific backend checks evident in the text.',
...
export async function POST(req: Request) {
  try {
    ...
    const pdfData = extractTextFromPdfBuffer(buffer);
