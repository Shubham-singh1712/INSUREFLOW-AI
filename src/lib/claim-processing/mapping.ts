...
const buildField = (id: string, label: string, field?: TraceableField<any>): UiClaimField => {
  ...
  const isExtracted = hasValue && String(field.value).trim().length > 0;
  const sourceStr = field?.page ? `Page ${field.page}` : isExtracted ? 'AI Pipeline' : 'Not extracted';
  return {
    id,
    label,
...
export const mapExtractedFieldsToUi = (
  ...
  const fields: UiClaimField[] = [
    ...
    buildField('clinical.icd10_codes', 'ICD-10 Codes', extracted?.clinical?.icd10_codes ? {
      ...extracted.clinical.icd10_codes,
      value: Array.isArray(extracted.clinical.icd10_codes.value)
        ? extracted.clinical.icd10_codes.value.join(', ')
        : String(extracted.clinical.icd10_codes.value || ''),
    } : undefined),
