...
import {
  ScanLine, FileSearch, PenLine, Eye, ShieldCheck, BarChart2,
  CheckCircle2, Zap, ArrowLeft, ArrowRight, Database,
} from 'lucide-react';
...
const extractionPhases = [
  { id: 'phase-ocr', label: 'Deep OCR Text Extraction', icon: ScanLine, duration: 1800, target: 'All documents' },
  { id: 'phase-nlp', label: 'NLP Entity Recognition', icon: FileSearch, duration: 1600, target: 'Patient & Insurance fields' },
  { id: 'phase-codes', label: 'Medical Code Extraction', icon: PenLine, duration: 1400, target: 'ICD-10 & CPT/HCPCS codes' },
  { id: 'phase-billing', label: 'Billing Line Item Parsing', icon: Eye, duration: 1200, target: 'Itemized bill charges' },
  { id: 'phase-clinical', label: 'Clinical Data Mapping', icon: ShieldCheck, duration: 1000, target: 'Discharge summary & EHR' },
  { id: 'phase-schema', label: 'DB Schema Mapping', icon: Database, duration: 900, target: 'Claim extraction record' },
  { id: 'phase-confidence', label: 'Confidence Scoring', icon: BarChart2, duration: 700, target: 'All extracted fields' },
];
...
export default function AIExtractionStep({ uploadedDocs, onComplete, onBack }: AIExtractionStepProps) {
      ...
      if (idx >= extractionPhases.length) {
        fetch('/api/claims/extract', {
          ...
          .then(({ response, payload }) => {
            setExtractedData(response.ok && payload.ok ? payload.data.extractedData : mockExtractedData);
  ...
  const displayData = extractedData || mockExtractedData;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="card p-5">
          <div className="flex items-center gap-3 mb-5">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${
              extractionDone ? 'bg-success-bg' : 'bg-primary/10'
            }`}>
              {extractionDone
            ...
            <div>
              <h3 className="font-semibold text-foreground text-sm">
                {extractionDone ? 'Full AI Extraction Complete' : 'Deep AI Extraction Running'}
              </h3>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 p-2.5 rounded-xl transition-all duration-300 ${
                    ...
                    isCompleted ? 'opacity-80' : 'opacity-35'
                  }`}
                >
                  ...
                  {isActive && <span className="text-xs text-primary font-medium validation-pulse shrink-0">Running</span>}
                  {isCompleted && <span className="text-xs text-success font-medium shrink-0">Done</span>}
                </div>
              );
            })}
          </div>
        ...
        {extractionDone && (
          <div className="card p-5 fade-in">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {displayData.extraction_meta.low_confidence_fields.length} fields flagged for manual review
            </p>
          </div>
        )}
      </div>
        ...
        {!extractionDone ? (
          <div className="card p-8 flex flex-col items-center justify-center text-center min-h-[300px]">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">
              AI is extracting and mapping all document fields to the standardized claim database schema...
            </p>
          </div>
        ) : (
          <div className="space-y-4 fade-in">
            ...
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                {[
                  ...
                  { label: 'Date of Birth', value: displayData.patient.date_of_birth, confidence: 0.97 },
                  { label: 'Member ID', value: displayData.insurance.member_id, confidence: 0.95 },
                  { label: 'Payer ID', value: displayData.insurance.payer_id, confidence: 0.71, low: true },
                  { label: 'Group Number', value: displayData.insurance.group_number, confidence: 0.93 },
                  { label: 'Plan Name', value: displayData.insurance.plan_name, confidence: 0.96 },
                ].map((field) => (
                  <div key={`ef-${field.label}`} className="flex items-start justify-between gap-2 py-1.5 border-b border-border/50 last:border-0">
                    <span className="text-xs text-muted-foreground shrink-0">{field.label}</span>
                    <div className="text-right">
                      <p className={`text-xs font-medium ${field.low ? 'text-warning-foreground' : 'text-foreground'} font-tabular`}>
                        {field.value}
                      </p>
                      <p className={`text-xs ${field.low ? 'text-warning-foreground' : 'text-success-foreground'}`}>
            ...
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-3">
                {[
                  { label: 'Admission', value: displayData.clinical.admission_date, confidence: 0.98 },
                  { label: 'Discharge', value: displayData.clinical.discharge_date, confidence: 0.98 },
                  { label: 'Physician', value: displayData.clinical.attending_physician, confidence: 0.95 },
                  { label: 'Hospital NPI', value: displayData.clinical.hospital_npi, confidence: 0.99 },
                ].map((field) => (
                  <div key={`cf-${field.label}`} className="flex items-start justify-between gap-2 py-1.5 border-b border-border/50 last:border-0">
                    <span className="text-xs text-muted-foreground shrink-0">{field.label}</span>
                    <div className="text-right">
                      <p className="text-xs font-medium text-foreground font-tabular">{field.value}</p>
                      <p className="text-xs text-success-foreground">{Math.round(field.confidence * 100)}% conf.</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="space-y-1.5">
                {displayData.coding.icd10_codes.map((code) => (
                  <div key={`icd-${code.code}`} className="flex items-center gap-2 py-1 px-2 bg-muted/50 rounded-lg">
                    <span className="badge-info text-xs font-tabular">ICD-10</span>
                    <span className="text-xs font-bold text-foreground font-tabular">{code.code}</span>
                    <span className="text-xs text-muted-foreground truncate flex-1">{code.description}</span>
                    <span className="text-xs text-success-foreground shrink-0">{Math.round(code.confidence * 100)}%</span>
                  </div>
                ))}
                {displayData.coding.cpt_codes.map((code) => (
                  <div key={`cpt-${code.code}`} className={`flex items-center gap-2 py-1 px-2 rounded-lg ${
                    code.confidence < 0.85 ? 'bg-warning-bg/40' : 'bg-muted/50'
                  }`}>
                    <span className="badge-warning text-xs font-tabular">CPT</span>
                    <span className="text-xs font-bold text-foreground font-tabular">{code.code}</span>
                    <span className="text-xs text-muted-foreground truncate flex-1">{code.description}</span>
                    <span className={`text-xs shrink-0 ${code.confidence < 0.85 ? 'text-warning-foreground' : 'text-success-foreground'}`}>
            ...
            <div className="card p-5">
              ...
              <div className="space-y-1.5 mb-3">
                {displayData.billing.line_items.map((item, i) => (
                  <div key={`li-${i}`} className="flex items-center justify-between gap-2 py-1 text-xs">
                    <span className="text-muted-foreground flex-1">{item.description}</span>
                    <span className="text-foreground font-tabular shrink-0">₹{parseInt(item.gross_charge).toLocaleString()}</span>
                  </div>
                ))}
              </div>
      ...
      <div className="lg:col-span-5 card p-5 flex items-center justify-between">
        ...
        <button
          onClick={() => onComplete(displayData)}
