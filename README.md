...
Deploy the frontend from the `insureflowai` folder.
...
Required environment variables are listed in `.env.example`. Add them in Vercel Project Settings > Environment Variables. Do not commit real API keys.

### Render

The repository includes a root-level `render.yaml` blueprint with two services:

- `insureflowai-web`: Next.js frontend from `insureflowai`
- `insureflowai-api`: Express backend from `insureflowai/backend`
