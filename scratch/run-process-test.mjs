...
import { processDemoClaimPipeline } from '../src/lib/claim-processing/demo-pipeline';
...
const claimId = 'claim-test-process-123';
const uploadSessionId = 'session-test-process-123';
...
async function run() {
  ...
  try {
    const packet = await processDemoClaimPipeline(buffer, sessionData);
    console.log('Demo pipeline completed successfully. Packet state:', packet.state);
    ...
    console.error('Pipeline test threw error:', err);
  }
}
